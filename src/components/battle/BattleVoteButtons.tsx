  ...
  return (
    <div className="absolute bottom-4 left-0 right-0 px-4">
      <div className="flex space-x-3">
        <Button 
          className={`flex-1 ${votedFor === 'user1' ? 'bg-app-yellow text-app-black' : 'bg-black/50 text-white'} rounded-full`}
          onClick={() => onVote('user1')}
        >
          Vote {user1Name}
        </Button>
        <Button 
          className={`flex-1 ${votedFor === 'user2' ? 'bg-app-yellow text-app-black' : 'bg-black/50 text-white'} rounded-full`}
